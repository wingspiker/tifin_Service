  ...
  try {
    ...
    const menuDetailsPromises = menus.map(async (menu) => {
      const menuItem = await Menu.findOne({
        where: { id: menu.menu_id, isPublished: true },
      });
      if (!menuItem) {
        throw new Error(
          `Menu item with ID ${menu.menu_id} not found or not published.`
        );
      }
...
exports.getAllAddresses = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({
        error: error.message || "An error occurred while retrieving addresses",
      });
  }
};
  ...
  try {
    ...
    if (orders.length === 0) {
      return res
        ...
        .json({
          message:
            "No orders found for the given mobile number or all orders are completed.",
        });
    }
    ...
    const orderDetailsPromises = orders.map(async (order) => {
      ...
      const menuItems = await Promise.all(
        order.menus.map(async (menu) => {
          ...
          return {
            // Include all properties from menuItem
            id: menuItem.id,
            date: menuItem.date,
            photo_url: menuItem.photo_url,
            isPublished: menuItem.isPublished,
    ...
    return res
      ...
      .json({
        error:
          error.message || "An error occurred while retrieving order details",
      });
  }
};
...
exports.initiatePayment = async (req, res) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      return res
        .status(400)
        .json({ message: "Payment initiation failed", data: response.data });
    }
  } catch (error) {
...
exports.checkPaymentStatus = async (req, res) => {
  ...
  try {
    ...
    if (response.data.code == "PAYMENT_SUCCESS") {
      ...
      return res
        .status(200)
        .json({ message: "Payment successfull", data: payment });
    } else {
      const payment = await Payment.create({
        order_id: order_id,
        payment_id: response.data.data.transactionId,
...
exports.getAllMedia = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to retrieve media records" });
  }
};
